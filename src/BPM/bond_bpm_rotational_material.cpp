...
#include "bond_bpm_rotational_material.h"
...
BondBPMRotationalMaterial::BondBPMRotationalMaterial(LAMMPS *_lmp) :
...
BondBPMRotationalMaterial::~BondBPMRotationalMaterial()
{
  delete[] svector;
...
double BondBPMRotationalMaterial::store_bond(int n, int i, int j)
{
...
void BondBPMRotationalMaterial::store_data()
{
  int i, j, m, type;
  double delx, dely, delz, r, rinv;
  double **x = atom->x;
...
double BondBPMRotationalMaterial::elastic_forces(int i1, int i2, int type, double r_mag, double r0_mag,
                                         ...
                                         double *force1on2, double *torque1on2, double *torque2on1)
{
  ...
  Fr = Kr[type] * r0_mag * (r_mag - r0_mag);
...
void BondBPMRotationalMaterial::damping_forces(int i1, int i2, int type, double *rhat, double *r,
                                       double *force1on2, double *torque1on2, double *torque2on1)
{
...
double BondBPMRotationalMaterial::calculate_heat(int type, double Ti, double Tj, double l, double l0)
{
  return Kh[type] * l0 * l0 * (Tj - Ti) / l / l;
}

double BondBPMRotationalMaterial::tempbreak(int type, double Ti, double Tj) 
{
  double Tmax = Ti;
  if (Tj > Tmax) Tmax = Tj;
  return Tmax / Fch[type];
}

void BondBPMRotationalMaterial::compute(int eflag, int vflag)
{
  ...
  for (n = 0; n < nbondlist; n++) {
    ...
    if (heat_flag) {
      ...
      dq = calculate_heat(type, Ti, Tj, r_mag, r0_mag);

      double hbreaking; 
      hbreaking = tempbreak(type, Ti, Tj);
      breaking += hbreaking;
    }
...
void BondBPMRotationalMaterial::allocate()
{
  allocated = 1;
  ...
  if (heat_flag) {
    ...
    memory->create(Fch, np1, "bond:Fch");
  }
...
void BondBPMRotationalMaterial::coeff(int narg, char **arg)
{
  int coeffnarg = 13;
  ...
  double Fch_one = 0.0;
  if (heat_flag) {
    Kh_one = utils::numeric(FLERR, arg[13], false, lmp);
    Fch_one = utils::numeric(FLERR, arg[14], false, lmp);
  }
  if (Fcr_one == 0)
    fprintf(screen,"WARNING: bpm/rotational critical bond stretch, Fcr, must be non-zero.\n");
  if (Fcs_one == 0)
    fprintf(screen,"WARNING: bpm/rotational critical bond shear, Fcs, must be non-zero.\n");
  if (Tct_one == 0)
    fprintf(screen,"WARNING: bpm/rotational critical bond twist, Tct, must be non-zero.\n");
  if (Tcb_one == 0)
    fprintf(screen,"WARNING: bpm/rotational critical bond bend, Tcb, must be non-zero.\n");
...
void BondBPMRotationalMaterial::init_style()
{
  BondBPM::init_style();
...
void BondBPMRotationalMaterial::settings(int narg, char **arg)
{
...
void BondBPMRotationalMaterial::write_restart(FILE *fp)
{
...
void BondBPMRotationalMaterial::read_restart(FILE *fp)
{
...
void BondBPMRotationalMaterial::write_restart_settings(FILE *fp)
{
...
void BondBPMRotationalMaterial::read_restart_settings(FILE *fp)
{
...
double BondBPMRotationalMaterial::single(int type, double rsq, int i, int j, double &fforce)
{
  // Not yet enabled
