...
#include "atom_vec_bpm_sphere_temp.h"
...
AtomVecBPMSphereTemp::AtomVecBPMSphereTemp(LAMMPS *_lmp) : AtomVec(_lmp)
{
  mass_type = PER_ATOM;
  forceclearflag = 1;
  ...
  fields_grow = {"molecule", "num_bond", "bond_type", "bond_atom", "nspecial", "special",
                 "radius",   "rmass",    "omega",     "torque",    "quat", "temperature", "heatflow"};
  fields_copy = {"molecule", "num_bond", "bond_type", "bond_atom", "nspecial",
                 "special",  "radius",   "rmass",     "omega",     "quat", "temperature", "heatflow"};
  fields_comm_vel = {"omega", "quat", "temperature", "heatflow"};
  fields_reverse = {"torque", "heatflow"};
  fields_border = {"molecule", "radius", "rmass", "temperature", "heatflow"};
  fields_border_vel = {"molecule", "radius", "rmass", "omega", "quat", "temperature", "heatflow"};
  fields_exchange = {"molecule", "num_bond", "bond_type", "bond_atom", "nspecial",
                     "special",  "radius",   "rmass",     "omega",     "quat", "temperature", "heatflow"};
  fields_restart = {"molecule", "num_bond", "bond_type", "bond_atom",
                    "radius",   "rmass",    "omega",     "quat", "temperature", "heatflow"};
  fields_create = {"molecule", "num_bond", "nspecial", "radius", "rmass", "omega", "quat", "temperature", "heatflow"};
  fields_data_atom = {"id", "molecule", "type", "radius", "rmass", "x", "temperature"};
...
void AtomVecBPMSphereTemp::process_args(int narg, char **arg)
{
...
void AtomVecBPMSphereTemp::init()
{
  AtomVec::init();
...
void AtomVecBPMSphereTemp::grow_pointers()
{
  radius = atom->radius;
  rmass = atom->rmass;
  omega = atom->omega;
...
void AtomVecBPMSphereTemp::force_clear(int n, size_t nbytes)
{
...
void AtomVecBPMSphereTemp::create_atom_post(int ilocal)
{
  radius[ilocal] = 0.5;
...
void AtomVecBPMSphereTemp::pack_restart_pre(int ilocal)
{
...
void AtomVecBPMSphereTemp::pack_restart_post(int ilocal)
{
...
void AtomVecBPMSphereTemp::unpack_restart_init(int ilocal)
{
  nspecial[ilocal][0] = 0;
  nspecial[ilocal][1] = 0;
  nspecial[ilocal][2] = 0;
...
void AtomVecBPMSphereTemp::data_atom_post(int ilocal)
{
...
void AtomVecBPMSphereTemp::pack_data_pre(int ilocal)
{
...
void AtomVecBPMSphereTemp::pack_data_post(int ilocal)
{
  radius[ilocal] = radius_one;
